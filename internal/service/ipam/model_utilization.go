...
var UtilizationAttrTypes = map[string]attr.Type{
...
var UtilizationResourceSchemaAttributes = map[string]schema.Attribute{
...
func ExpandUtilization(ctx context.Context, o types.Object, diags *diag.Diagnostics) *ipam.Utilization {
	if o.IsNull() || o.IsUnknown() {
		return nil
	}
	var m UtilizationModel
...
func (m *UtilizationModel) Expand(ctx context.Context, diags *diag.Diagnostics) *ipam.Utilization {
	if m == nil {
		return nil
	}
	to := &ipam.Utilization{}
...
func FlattenUtilization(ctx context.Context, from *ipam.Utilization, diags *diag.Diagnostics) types.Object {
	if from == nil {
		return types.ObjectNull(UtilizationAttrTypes)
	}
	m := UtilizationModel{}
	m.Flatten(ctx, from, diags)
	t, d := types.ObjectValueFrom(ctx, UtilizationAttrTypes, m)
	diags.Append(d...)
	return t
}

func (m *UtilizationModel) Flatten(ctx context.Context, from *ipam.Utilization, diags *diag.Diagnostics) {
	if from == nil {
		return
	}
	if m == nil {
	...
	m.AbandonUtilization = flex.FlattenInt64Pointer(from.AbandonUtilization)
